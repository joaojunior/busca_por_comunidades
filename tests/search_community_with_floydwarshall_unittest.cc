...
TEST(SearchComunityTest, VerifyQuantityShortestPathInEdgeWithFloydWarshall2){
    Graph graph;
    ResultFloydWarshall *floyd_warshall_result;
    int **quantity_shortest_path_in_edge;
...
TEST(SearchComunityTest, CalculateCommunitiesWithFloydWarshallWith1Community){
    Graph graph;
    ...
    insert_edge(&graph, 1, 2, 1);
    insert_edge(&graph, 2, 3, 1);
    communities2nodes = calculate_communities_floyd_warshall(&graph, 1);
